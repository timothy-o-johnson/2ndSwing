 ...
 */ function (record, search, runtime) {
   ...
   */
    function getInputData () {
      ...
      if (length > 0) {
        log.debug('searchObj results:', JSON.stringify(searchObj))
        return searchObj
      } else {
    ...
    function map (context) {
      ...
      do {
        ...
        // expenseaccount *is* cogsaccount
          ...
          newItem.setValue({
            fieldId: 'class',
            value: poType.class.hasOwnProperty('value')
              ? poType.class[0].value
              : 2
          })
          ...
          try {
            ...
            log.error({
              title: 'FIELDSET ERROR custitem_g2_itemtype_ref',
          ...
          try {
            ...
            log.error({
              title: 'FIELDSET ERROR custitem_g2_model_ref',
              details: JSON.stringify(eX)
            })
          }
          try {
            ...
            log.error({
              title: 'FIELDSET ERROR custitem_g2_brand_ref',
              details: JSON.stringify(eX)
            })
          }
          try {
            ...
            log.error({
              title: 'FIELDSET ERROR custitem_g2_category_ref',
              details: JSON.stringify(eX)
            })
          }
          ...
          try {
            ...
            log.error({
              title: 'FIELDSET ERROR custitem_g2_condition_ref',
              details: JSON.stringify(eX)
            })
          }
          ...
          log.debug('basePriceLine', basePriceLine)
          ...
          log.debug('aa msrpLine', msrpLine)
      ...
      try {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          do {
            ...
            log.debug(
              'after Rate and qty set',
              rate + ' ' + qty + ' ' + poFields.length + ' ' + 'item'
            )
            ...
            for (var i_col = 0; i_col < poFields.length; i_col++) {
              if (
                poFields[i_col].indexOf('custcol_') == 0 &&
              poFields[i_col] != 'custcol_expectedqty'
              ) {
                log.debug({
                  title:
                  'Setting ' + poFields[i_col] + ' line ' + String(lineNum),
                  details: po.getSublistValue({
                    sublistId: 'item',
                ...
                if (poFields[i_col] == 'custcol_wipfli_g2in') {
                  custcol_wipfli_g2in = po.getSublistValue({
                    sublistId: 'item',
                    fieldId: poFields[i_col],
                    line: lineNum
                  })
   ...
   */
    ...
    function summarize (summary) {
      ...
      if (summary.inputSummary.error) {
        log.debug('input errors', JSON.stringify(summary.inputSummary.error))
      }
      if (summary.mapSummary.error) {
        log.debug('map errors', JSON.stringify(summary.mapSummary.error))
      }
      if (summary.reduceSummary.error) {
        log.debug('reduce errors', JSON.stringify(summary.reduceSummary.error))
      }
