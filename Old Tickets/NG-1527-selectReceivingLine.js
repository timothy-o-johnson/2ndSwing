		...
		try {
			if (context.request.method === 'POST') {
				...
				if (params['custpage_rf_orderid'] && params['custpage_rf_orderid'].length > 0) {
					...
					receivingClass = POrec.getValue({fieldId: 'class'})
					receivingClassText = POrec.getText({fieldId: 'class'});
					WMSNotes = POrec.getValue({fieldId: 'custbody_wipfli_wms_notes'});
					tranid = POrec.getValue({fieldId : 'tranid'});
					...
					receiveOrder_search.run().each(function(result){
						...
						//you are iterating through the hash table anyways, so going to just use the key as the counter
						//itemLinesHash[objTemp.lineid] = objTemp;
						//fix for itemLinesHash
						itemLinesHash[ctr + 1] = objTemp;
					...
					log.debug('how many items does the search think we have?', ctr);
					...
					log.debug('receivingClass here',receivingClass);
					...
					for(var i = totalLines; i > 0; i--){
						log.debug("itemLinesHash.hideinwms " + i, itemLinesHash[i]);
						...
						if (item.hasOwnProperty('docId')) {
							...
							for( var f = 0; f < cols.length; f++){
								if(itemFields[cols[f]].length > 0 && getValuesFromPO == false){
									...
									var lineItemNum = i-1;
									// TJ
									// log.debug('vf line', lineItemNum);
									// log.debug('column trying to access', item.fieldData[f].custrecord_fmd_columnscriptid);
									
									if(item.fieldData[f].custrecord_fmd_columnscriptid.id != ""){
										item.fieldData[f].value = POrec.getSublistValue({
											sublistId : 'item',
											line : lineItemNum,
											fieldId : item.fieldData[f].custrecord_fmd_columnscriptid.id
										});
									//	log.debug('before trying to get text', lineItemNum);
									...
									if(item.fieldData[f].custrecord_fmd_po_col_scriptid.id != ""){
					...
					for (var l = attrArrayer.length; l >=0 ; l--) {
						...
						var templateLine = {
							...
							receivingNotes : attrArrayer[l].receivingnotes,
					...
					if (receivingClass == 2) {
						...
						log.debug('receivingClass', receivingClass);
						var addItemData = {
